...
async function getButtonScript(): Promise<{|
  Buttons: (Object) => typeof ElementNode,
...
test(`Tag should be applied with no design for control`, async () => {
  ...
  if (!designSignal || designScript) {
    throw new Error(
      "Expected to find a data-design-experiment tag but no script"
    );
  }
});
...
test(`Tag should be applied with no design for control`, async () => {
  ...
  if (designSignal || designScript) {
    throw new Error(
      "Expected not to find a data-design-experiment tag and no script"
    );
  }
});
