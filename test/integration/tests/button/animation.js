...
describe('Tests for button animations and designs', () => {
    ...
    describe('Tests for fadeout logo and show label text animation', () => {
        beforeEach(() => {
            createTestContainer();
        });
            ...
            window.paypal.Buttons({
                ...
                test: {
                    onRender() {
                        ...
                        // $FlowFixMe
                        const win = frame.contentWindow;
                        const animationContainer = win.document.querySelector('.fadeout-logo-show-label-animation-container');
                        if (animationContainer) {
                            done(new Error('No personalization should be applied on __WEB__'));
                        }
                        done();
