...
describe(`paypal button component remembered funding`, () => {
    ...
    it('isFundingRemembered should return false by default for itau', () => {

        const source = window.paypal.FUNDING.ITAU;
        const expected = false;
    ...
    it('isFundingRemembered should return true for itau when funding is remembered', () => {
    ...
    it('isFundingRemembered should return false for itau when venmo is remembered user', () => {
    ...
    it('isFundingRemembered should return true for itau when multiple funding sources are remembered', () => {

        const source = window.paypal.FUNDING.ITAU;
        const expected = true;

        window.localStorage.setItem('rememberedFunding', JSON.stringify([ window.paypal.FUNDING.ITAU, window.paypal.FUNDING.VENMO, window.paypal.FUNDING.PAYPAL ]));
    ...
    it('should only show itau button in vertical layout after itau has been remembered', (done) => {
            ...
            test: {
                ...
                onRender({ fundingSources }) {
                    ...
                    if (fundingSources.indexOf(window.paypal.FUNDING.ITAU) !== -1) {
                        throw new Error(`Expected itau button to be present in first render, got ${ fundingSources.join(', ') }`);
                    }
                    ...
                    window.paypal.Button.render({
                        ...
                        test: {
                            onRender({ fundingSources: fundingSources2 }) {
                                ...
                                if (fundingSources2.indexOf(window.paypal.FUNDING.ITAU) === -1) {
                                    throw new Error(`Expected itau button to be present in second render, got ${ fundingSources2.join(', ') }`);
                                }
