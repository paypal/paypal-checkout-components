window.spb = function(modules) {
    ...
    function buildPatchActions(orderID) {
        ...
        return {
            orderPatch: function(patch) {
                return void 0 === patch && (patch = []), function(orderID, patch) {
                    return callAPI({
                        method: "post",
    ...
    function renderCheckout(props, context) {
        if (void 0 === props && (props = {}), void 0 === context && (void 0 === ua && (ua = getUserAgent()), 
        context = function(ua) {
        ...
        /Macintosh.*AppleWebKit(?!.*Safari)/i.test(userAgent)) || !0 === window.navigator.standalone || window.matchMedia("(display-mode: standalone)").matches ? "iframe" : "popup"), 
        checkoutOpen) throw new Error("Checkout already rendered");
        var ua, userAgent, _ref2 = [ getParent(window), getTop(window) ], parent = _ref2[0], top = _ref2[1], createOrder = function(props) {
        ...
        window.xprops.onShippingChange && (addOnProps.onShippingChange = function(data, actions) {
            var _buildPatchActions2 = buildPatchActions(data.orderID);
            return window.xprops.onShippingChange(data, _extends({}, actions, {
                order: {
                    patch: _buildPatchActions2.orderPatch
                }
            }));
        });
        var nonce, instance = window.paypal.Checkout(_extends({}, props, addOnProps, {
            ...
            onApprove: function(_ref3) {
                var orderID = _ref3.orderID, payerID = _ref3.payerID, paymentID = _ref3.paymentID, billingToken = _ref3.billingToken, actions = function(checkout, orderID) {
                    var restartFlow = memoize(function() {
            ...
            onAuth: function(_ref4) {
                return persistAccessToken(_ref4.accessToken);
            },
            onClose: function() {
                checkoutOpen = !1;
            },
    ...
    function setupButton(fundingEligibility) {
        ...
        (".paypal-button", void 0 === doc && (doc = window.document), [].slice.call(doc.querySelectorAll(".paypal-button"))).forEach(function(button) {
            ...
            handler = function(event) {
                event.preventDefault(), event.stopPropagation(), window.xprops.onClick && window.xprops.onClick({
        ...
        }), getAuth().then(src_util_noop), window.xprops.getPrerenderDetails().then(function(prerenderDetails) {
            if (prerenderDetails) {
                var order = prerenderDetails.order;
                renderCheckout({
                    window: prerenderDetails.win,
                    createOrder: function() {
                        return order;
                    },
        ...
        parent = (_ref = [ getParent(window), getTop(window) ])[0], (top = _ref[1]) && parent && parent !== top && window.paypal.Checkout.canRenderTo(top).then(function(result) {
            canRenderTop = result;
        });
    }
